...
**Instructions:**
1.  Analyze the provided network status. The status follow this format:
    1. **buses**: A list of all bus nodes in the network. Each bus includes:
       - `id`: Unique identifier (e.g., "Bus5")
       - `vn_kv`: Nominal voltage level in kilovolts
    ...
    3. **transformers**: A list of power transformers. Each transformer includes:
       ...
       - `tap_pos`: Tap position for voltage control
...
2.  Generate an optimized plan using these strategies:
    - Switch reconfigurations: Can reroute power to resolve multiple thermal and/or voltage violations
    - Strategic battery placement: Position batteries to support multiple thermal and/or voltage violations  
    - Coordinated load curtailment: Curtail loads that impact multiple violations

3. Prioritize actions in the following order: Switch reconfigurations, adding batteries, curtailing loads.
4. Minimize the total number of actions per iteration.
